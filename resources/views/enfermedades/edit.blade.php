...
@section('header')
    <h1 class="text-2xl font-bold">Editar Enfermedad Base</h1>
@endsection
    ...
    <div class="bg-white p-6 rounded shadow-md">
        <form action="{{ route('enfermedades.update', $enfermedad) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" id="nombre" class="border rounded w-full py-2 px-3" value="{{ old('nombre', $enfermedad->nombre) }}" required>
                @error('nombre') <p class="text-red-500">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label for="descripcion" class="block text-gray-700">Descripción</label>
                <textarea name="descripcion" id="descripcion" class="border rounded w-full py-2 px-3" rows="3">{{ old('descripcion', $enfermedad->descripcion) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <select name="CATA_Estado" id="CATA_Estado" class="border rounded w-full py-2 px-3" required>
                    ...
                    <option value="41" {{ old('CATA_Estado', $enfermedad->CATA_Estado) == 41 ? 'selected' : '' }}>Activo</option>
                    <option value="42" {{ old('CATA_Estado', $enfermedad->CATA_Estado) == 42 ? 'selected' : '' }}>Inactivo</option>
                </select>
