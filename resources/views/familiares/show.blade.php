    ...
    <div class="container mx-auto p-4">
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="bg-green-600 px-6 py-4 flex items-center">
                <div
                    class="flex-shrink-0 h-20 w-20 rounded-full bg-white flex items-center justify-center border-4 border-white">
            ...
            <div class="bg-gray-100 border-b border-gray-200 px-6 py-3">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('familiares.edit', $familiar) }}"
                        class="bg-yellow-500 hover:bg-yellow-600 text-white py-1 px-3 rounded-md text-sm flex items-center transition-colors">
                        <i class="fas fa-edit mr-1"></i> Editar
                    </a>
                    <a href="{{ route('familiares.medicamentos', $familiar) }}"
                        class="bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded-md text-sm flex items-center transition-colors">
                        <i class="fas fa-pills mr-1"></i> Historial de Medicamentos
                    </a>
                    <a href="{{ route('ordenes_medicas.indexByFamiliar', $familiar) }}"
                        class="bg-purple-500 hover:bg-purple-600 text-white py-1 px-3 rounded-md text-sm flex items-center transition-colors">
                        <i class="fas fa-file-medical mr-1"></i> Historial de Órdenes
                    </a>
                    <a href="{{ route('ordenes_medicas.create') }}?familiar_id={{ $familiar->id }}"
                        class="bg-orange-500 hover:bg-orange-600 text-white py-1 px-3 rounded-md text-sm flex items-center transition-colors">
                        <i class="fas fa-plus-circle mr-1"></i> Nueva Orden Médica
                    </a>
                    <a href="{{ route('familiares.index') }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white py-1 px-3 rounded-md text-sm flex items-center transition-colors">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        <div class="mb-6">
                            <h3
                                class="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-3 flex items-center">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-start">
                                    ...
                                    <div class="w-2/3 text-gray-900">{{ $familiar->genero->Valor1 ?? 'No especificado' }}
                                    </div>
                                </div>
                                <div class="flex items-start">
                        ...
                        <div>
                            ...
                            @php
                                $tieneContactos =
                                    !empty($familiar->contacto_nombre1) ||
                                    ...
                                    @if (!empty($familiar->contacto_nombre1) || !empty($familiar->contacto_telefono1))
                                        <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                            ...
                                            @if (!empty($familiar->contacto_telefono1))
                                                <a href="tel:{{ $familiar->contacto_telefono1 }}"
                                    ...
                                    @if (!empty($familiar->contacto_nombre2) || !empty($familiar->contacto_telefono2))
                                        <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                            ...
                                            @if (!empty($familiar->contacto_telefono2))
                                                <a href="tel:{{ $familiar->contacto_telefono2 }}"
                                                    class="text-blue-600 hover:underline flex items-center mt-1">
                                                    <i class="fas fa-phone-alt mr-1"></i>
                                    ...
                                    @foreach ($familiar->enfermedades as $enfermedad)
                                        <div class="border-l-4 border-orange-500 pl-4 py-1">
                                            ...
                                            @if (isset($enfermedad->pivot) && $enfermedad->pivot->notas)
                                                <div class="text-sm text-gray-600 mt-1">{{ $enfermedad->pivot->notas }}
                                                </div>
                                            @endif
                                        </div>
                                    @endforeach
                    ...
                    <div>
                                                    ...
                                                    <div class="flex items-center">
                                                        ...
                                                        <div class="ml-4">
                                                            <h4 class="text-lg font-medium text-gray-900">
                                                                {{ $medicamento->medicamento->Nombre }}</h4>
                                                            <p class="text-sm text-gray-600">
                                                                {{ $medicamento->descripcion_tratamiento }}</p>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="mt-3 pt-3 border-t border-gray-200 text-sm">
                                                    <div class="grid grid-cols-2 gap-2">
                                                        <div>
                                                            ...
                                                            <span
                                                                class="font-medium">{{ \Carbon\Carbon::parse($medicamento->fecha_inicio)->format('d/m/Y') }}</span>
                                                        </div>
                                                        <div>
                                                            @if ($medicamento->fecha_final)
                                                                ...
                                                                <span
                                                                    class="font-medium {{ $fechaVencida ? 'text-red-600' : '' }}">
                                                                    {{ \Carbon\Carbon::parse($medicamento->fecha_final)->format('d/m/Y') }}
                                                                </span>

                                                                @if ($diasRestantes > 0)
                                                                    <span
                                                                        class="ml-2 px-2 py-1 text-xs rounded-full {{ $diasRestantes <= 5 ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800' }}">
                                                            ...
                                                            @else
                                                                ...
                                                                <span class="font-medium text-blue-600">Tratamiento
                                                                    continuo</span>
                                                            @endif
                                                        </div>
                                                    </div>
                        ...
                        <div class="mt-8 bg-white rounded-lg shadow-md overflow-hidden">
                            ...
                            <div class="p-6">
                                @php
                                    $ordenesMedicas = $familiar->ordenesMedicas()->where('CATA_Estado', 41)->get();
                                @endphp
                                            ...
                                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                                                <div class="flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        <p class="text-sm text-gray-600">
                                                            <span class="font-medium">Centro Médico:</span>
                                                            {{ $orden->Centro_Medico }}, {{ $orden->Ciudad }}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <span
                                                ...
                                                <div class="mt-3 pt-3 border-t border-gray-200 text-sm">
                                                    <div class="grid grid-cols-2 gap-2">
                                                        <div>
                                                            ...
                                                            <span
                                                                class="font-medium">{{ \Carbon\Carbon::parse($orden->Fecha_Resetada)->format('d/m/Y') }}</span>
                                                        </div>
                                                        <div>
                                                            ...
                                                            @else
                                                                ...
                                                                <span class="font-medium text-orange-600">Sin cita
                                                                    programada</span>
                                                            @endif
                                                        </div>
                                                    </div>
