        ...
        <div class="bg-white p-6 rounded shadow-md">
            <form action="{{ route('familiares.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nombre" id="nombre" class="border rounded w-full py-2 px-3"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="apellido" id="apellido" class="border rounded w-full py-2 px-3"
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="fecha_nacimiento" id="fecha_nacimiento"
                        class="border rounded w-full py-2 px-3" value="{{ old('fecha_nacimiento') }}" required>
                    ...
                    <select name="CATA_tipo_sangre" id="CATA_tipo_sangre" class="border rounded w-full py-2 px-3">
                        ...
                        @foreach ($tipos_sangre as $tipo)
                            <option value="{{ $tipo->id }}"
                                {{ old('CATA_tipo_sangre') == $tipo->id ? 'selected' : '' }}>
                                {{ $tipo->Valor1 }}
                            </option>
                        @endforeach
                    </select>
                    @error('CATA_tipo_sangre')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="correo" id="correo" class="border rounded w-full py-2 px-3"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="contacto_nombre1" id="contacto_nombre1"
                        class="border rounded w-full py-2 px-3" value="{{ old('contacto_nombre1') }}">
                    @error('contacto_nombre1')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="contacto_telefono1" id="contacto_telefono1"
                        class="border rounded w-full py-2 px-3" value="{{ old('contacto_telefono1') }}">
                    @error('contacto_telefono1')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="contacto_nombre2" id="contacto_nombre2"
                        class="border rounded w-full py-2 px-3" value="{{ old('contacto_nombre2') }}">
                    @error('contacto_nombre2')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="contacto_telefono2" id="contacto_telefono2"
                        class="border rounded w-full py-2 px-3" value="{{ old('contacto_telefono2') }}">
                    @error('contacto_telefono2')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach ($enfermedades as $enfermedad)
                            <div class="mb-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="enfermedades[]" value="{{ $enfermedad->id }}"
                                        class="form-checkbox"
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-2 ml-4">
                        @foreach ($alergias as $alergia)
                            <div class="mb-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="alergias[]" value="{{ $alergia->id }}"
                                        class="form-checkbox"
                    ...
                    <select name="CATA_Estado" id="CATA_Estado" class="border rounded w-full py-2 px-3" required>
                        ...
                        @foreach ($estados as $estado)
                            <option value="{{ $estado->id }}"
                                {{ old('CATA_Estado') == $estado->id ? 'selected' : '' }}>
                                {{ $estado->Valor1 }}
                            </option>
                        @endforeach
                    </select>
                ...
                @if (auth()->user()->is_superadmin && $nucleosFamiliares)
                    <div class="mb-4">
                        <label for="nucleo_familiar_id" class="block text-gray-700">Núcleo Familiar</label>
                        <select name="nucleo_familiar_id" id="nucleo_familiar_id" class="border rounded w-full py-2 px-3"
                            required>
                            <option value="">Seleccione el núcleo familiar</option>
                            @foreach ($nucleosFamiliares as $nucleo)
                                <option value="{{ $nucleo->id }}"
                                    {{ old('nucleo_familiar_id') == $nucleo->id ? 'selected' : '' }}>
                                    {{ $nucleo->nombre }} ({{ $nucleo->codigo }})
                                </option>
                            @endforeach
                        </select>
                        @error('nucleo_familiar_id')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                @endif
                <div class="mb-4">
                    ...
                    <a href="{{ route('familiares.index') }}"
                        class="bg-gray-500 text-white py-2 px-4 rounded ml-2">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
