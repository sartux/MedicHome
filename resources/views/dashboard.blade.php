...
<div class="container mx-auto p-4">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden" x-data="{ expanded: true }">
        <div class="px-6 py-4 border-b border-gray-200 bg-green-50 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <!-- Próximas Citas -->
        <div class="bg-white rounded-lg shadow-md overflow-hidden" x-data="{ expanded: true }">
            <div class="px-6 py-4 border-b border-gray-200 bg-blue-50 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
                <h3 class="text-lg font-semibold text-gray-800">
                            ...
                            <div class="flex items-center p-3 rounded-lg border border-gray-200 hover:bg-blue-50">
                                ...
                                <div class="ml-3 flex-1">
                                    <div class="flex justify-between">
                                        <p class="text-sm font-medium text-gray-900">{{ $cita->familiar_nombre ?? 'Nombre Familiar' }}</p>
                                        <p class="text-xs text-gray-500">{{ $cita->fecha ?? '2025-03-15' }}</p>
                                    </div>
                                    <p class="text-xs text-gray-600">{{ $cita->especialidad ?? 'Especialidad' }} - {{ $cita->doctor ?? 'Dr. Ejemplo' }}</p>
                                </div>
                            </div>
                        @endforeach
                    @else
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden" x-data="{ expanded: true }">
            <div class="px-6 py-4 border-b border-gray-200 bg-orange-50 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
                <h3 class="text-lg font-semibold text-gray-800">
            ...
            <div x-show="expanded" x-transition:enter="transition ease-out duration-200" 
                ...
                <div class="space-y-3 max-h-64 overflow-y-auto">
                    ...
                    @if(count($citasPendientes) > 0)
                        @foreach($citasPendientes as $citaPendiente)
                            <div class="flex items-center p-3 rounded-lg border border-orange-200 hover:bg-orange-50">
                                ...
                                <div class="ml-3 flex-1">
                                    <div class="flex justify-between">
                                        <p class="text-sm font-medium text-gray-900">{{ $citaPendiente->familiar_nombre ?? 'Nombre Familiar' }}</p>
                                        <p class="text-xs text-orange-600 font-medium">Pendiente</p>
                                    </div>
                                    <p class="text-xs text-gray-600">{{ $citaPendiente->especialidad ?? 'Especialidad' }} - {{ $citaPendiente->procedimiento ?? 'Procedimiento' }}</p>
                                </div>
                            </div>
                        @endforeach
                    @else
                    ...
                    <div class="mt-4 text-center">
                        <a href="#" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-md transition-colors">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden col-span-1 md:col-span-2" x-data="{ expanded: true }">
            ...
            <div x-show="expanded" x-transition:enter="transition ease-out duration-200" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <a href="{{ route('familiares.create') }}" class="block py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg text-center transition duration-150 ease-in-out">
                        <i class="fas fa-user-plus mr-2"></i>Agregar Familiar
                    </a>
                    
                    <a href="{{ route('medicamentos.create') }}" class="block py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-center transition duration-150 ease-in-out">
                        <i class="fas fa-pills mr-2"></i>Nuevo Medicamento
                    </a>
                    
                    <a href="{{ route('historial_medicamentos.create') }}" class="block py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg text-center transition duration-150 ease-in-out">
